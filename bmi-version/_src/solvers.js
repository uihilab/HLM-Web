...
// 1. Vars, coefficients, etc
// --------------------------
export var RK4 = {"pow":(1/5),"a":[0.0, 0.5, 0.5, 1.0],"c":[0.0, 0.5, 0.5, 1.0]};

// -------------------
// 2. Solver Functions
// -------------------
export function RK4solver(forcings) {
	// 1. RK4 w/ full step
	// 2a. RK4 to half step
	...
	// 1. RK4 w/ full step
	var yTry = this.fsolve(Y, tUnix, this.hstep, forcings, true);
	// var yTry = this.fRK4(Y, tUnix, this.hstep, forcings, true);
	...
	var yHalf = this.fsolve(Y, tUnix, halfstep, forcings); // no ks returned

	// 2b. Use 2a to calc RK4 at full step
	var yWhole = this.fsolve(yHalf, tUnix + halfstep, halfstep, forcings); // no ks returned
	...
	for (i; i < yTry.length; i++){
		...
		let err = Math.abs(yTry[i] * this.solver.err.rel[i] / (yTry[i] - yWhole[i]));
	...
	if (this.hstep < hstep) {
		...
		yTry = this.fsolve(Y,tUnix,this.hstep,forcings,true);
		kTry = yTry.slice(1);
		yTry = yTry[0]; // min check in rk4 routine
...
export function fRK4(Y,tUnix,hstep,forcings,returnKs=false){
	...
	var a = RK4.a; var c = RK4.c; var limits = this.solver.limits.slice();
...
// 3. Beta functions
// -------------------
export function calcBetasRK4(theta){
