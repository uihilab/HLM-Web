...
// 1. Model section
// ---------------------------------
export function model190(Y,tUnix,f){
	...
	// dydt[0] = Math.pow(Y[0]/model.globalParams.q_r,model.globalParams.lambda_1) * this.invtau * dydt[0];
	dydt[0] = Math.pow(Y[0],model.globalParams.lambda_1) * this.invtau * dydt[0];
	return dydt;
};

export function model252(Y,tUnix,f){
	...
	function kt(k2,st){
		return k2 * (model.globalParams.A + model.globalParams.B * Math.pow((1 - st/model.globalParams.S_L),model.globalParams.exponent));
	};
	...
	qts = this.ki * Y[2];
	qsc = model.globalParams.k_3 * Y[3];

	dydt[0] = this.c2 * (qpc + qsc) - Y[0];
	dydt[0] += this.yFromParents(tUnix,0) // add q inflow from parents
	dydt[0] = this.invtau * Math.pow(Y[0],model.globalParams.lambda_1) * dydt[0];
	...
	dydt[2] = qpt - qts - f[1];
	dydt[3] = qts - qsc - f[2];
	dydt[6] = (model.globalParams.v_B / this.L) * 
...
// 2. Evap section
// ---------------------------------
export function getEvap190(t = this.linkTime){
	...
	var month = new Date(model._startDt.getTime() + t*1000).getMonth();
	var e_pot = this.evap[month] * model.globalParams.u; 
	// var e_pot = model.evapTable[month] * model.globalParams.u; 
...
export function getEvap252(t = this.linkTime){
	// return the calculated monthly evaporation
	// from ponded and subsurface water
	// evapPond and evapSub
	...
	a = this.state[0]/model.globalParams.s_r;
	b = this.state[2]/model.globalParams.S_L;
	c = this.state[3]/(model.globalParams.h_b - model.globalParams.S_L);
	corr = a + b + c;

	var month = new Date(model._startDt.getTime() + t*1000).getMonth();
	var e_pot = this.evap[month] * model.globalParams.u / corr;
	// var e_pot = model.evapTable[month] * model.globalParams.u / corr; 
	evap[0] = a * e_pot;
